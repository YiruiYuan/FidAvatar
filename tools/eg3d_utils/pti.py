...
def _build_side_back_consistency_cameras(
    device: torch.device,
    back_delta_deg: float = 30.0,
    radius: float = 2.75,
):
    lookat_position = torch.tensor([0, 0, 0], device=device)
    intrinsics = torch.tensor([[4.2647, 0, 0.5], [0, 4.2647, 0.5], [0, 0, 1]], device=device)

    back_delta = math.radians(float(np.clip(back_delta_deg, 1.0, 89.0)))
    pitch = math.pi / 2

    yaw_side_l = math.pi / 2 + math.pi / 2
    yaw_side_r = math.pi / 2 + 3 * math.pi / 2
    yaw_back_l = math.pi / 2 + math.pi - back_delta
    yaw_back_r = math.pi / 2 + math.pi + back_delta

    def _pack_camera(yaw):
        ...
        return torch.cat([cam2world.reshape(-1, 16), intrinsics.reshape(-1, 9)], 1)

    return {
        "side_l": _pack_camera(yaw_side_l),
        "side_r": _pack_camera(yaw_side_r),
        "back_l": _pack_camera(yaw_back_l),
        "back_r": _pack_camera(yaw_back_r),
    }


def _to_lowfreq_gray(image: torch.Tensor, pool_kernel: int = 8):
    image = (image + 1.0) * 0.5
    gray = image.mean(dim=1, keepdim=True)
    k = max(int(pool_kernel), 1)
    if k > 1:
        gray = F.avg_pool2d(gray, kernel_size=k, stride=k)
    return gray


def _compute_side_back_consistency_loss(
    G,
    ws: torch.Tensor,
    cameras,
    pool_kernel: int = 8,
):
    side_l = G.synthesis(ws, c=cameras["side_l"], noise_mode='const')['image']
    side_r = G.synthesis(ws, c=cameras["side_r"], noise_mode='const')['image']
    back_l = G.synthesis(ws, c=cameras["back_l"], noise_mode='const')['image']
    back_r = G.synthesis(ws, c=cameras["back_r"], noise_mode='const')['image']

    side_l_gray = _to_lowfreq_gray(side_l, pool_kernel=pool_kernel)
    side_r_gray = torch.flip(_to_lowfreq_gray(side_r, pool_kernel=pool_kernel), dims=[3])
    back_l_gray = _to_lowfreq_gray(back_l, pool_kernel=pool_kernel)
    back_r_gray = torch.flip(_to_lowfreq_gray(back_r, pool_kernel=pool_kernel), dims=[3])

    side_consistency_loss = F.l1_loss(side_l_gray, side_r_gray)
    back_consistency_loss = F.l1_loss(back_l_gray, back_r_gray)

    return side_consistency_loss, back_consistency_loss
...
def project_multi_view(
    ...
    lambda_side_back_consistency = 0.25,
    side_back_pool_kernel      = 8,
    side_back_delta_deg        = 30.0,
):
    ...
    consistency_cameras = None
    if lambda_side_back_consistency > 0:
        consistency_cameras = _build_side_back_consistency_cameras(
            device=device,
            back_delta_deg=side_back_delta_deg,
            radius=2.75,
        )
    ...
    for step in range(num_steps + 1):
        ...
        side_back_consistency_loss = torch.tensor(0.0, device=device)
        side_consistency_loss = torch.tensor(0.0, device=device)
        back_consistency_loss = torch.tensor(0.0, device=device)
        if consistency_cameras is not None:
            side_consistency_loss, back_consistency_loss = _compute_side_back_consistency_loss(
                G,
                ws,
                cameras=consistency_cameras,
                pool_kernel=side_back_pool_kernel,
            )
            side_back_consistency_loss = side_consistency_loss + back_consistency_loss

        loss = lambda_mse * mse_loss + \
                lambda_perc * perc_loss +  \
                lambda_w_norm * w_norm_loss + \
                regularize_noise_weight * reg_loss + \
                lambda_side_back_consistency * side_back_consistency_loss
        ...
        if step % 50 == 0:
            ...
            logprint(
                f'step: {step:>4d}/{num_steps} mse: {mse_loss:<4.2f} perc: {perc_loss:<4.2f} '
                f'w_norm: {w_norm_loss:<4.2f} noise: {float(reg_loss):<5.2f} '
                f'side: {float(side_consistency_loss):<4.2f} back: {float(back_consistency_loss):<4.2f}'
            )
...
def project_pti_multi_view(
    ...
    lambda_perc                = 1,
    lambda_clip                = 0.1,
    lambda_side_back_consistency = 0.15,
    side_back_pool_kernel      = 8,
    side_back_delta_deg        = 30.0,
):
